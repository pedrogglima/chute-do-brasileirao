...
RSpec.describe(Cache::RankingsService, type: :service) do
  let(:global_setting) { create(:global_setting) }
  let(:championship) { global_setting.championship }
  let(:cache_resources_service) do
    Cache::RankingsService.new({ current_championship_id: championship.id })
  end

  let(:key) { Cache::RankingsService::KEY }
  let(:exp) { Cache::RankingsService::EXP }
  let(:from) { Cache::RankingsService::FROM }
  let(:to) { Cache::RankingsService::TO }
    ...
    describe '#call' do
      ...
      context 'when resources do exist' do
        it 'should return all list' do
          create_resources
          cache_resources = cache_resources_service.call
          expect(cache_resources.length).to eq(20)
        end
      end
    end
  end
